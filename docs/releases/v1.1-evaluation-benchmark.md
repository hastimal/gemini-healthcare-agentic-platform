...
```mermaid
flowchart LR
    D["Frozen 11-Case<br/>Benchmark"]

    G["Gemini<br/>3.7 Flash"]
    M["Gemma 4 12B<br/>Ollama"]

    P["Google ADK<br/>3-Agent Platform"]

    R["Healthcare Retrieval<br/>NPPES · PubMed · FHIR"]
    S["Structured<br/>Pipeline Output"]

    A["Evaluation<br/>Adapter"]
    E["Deterministic Metrics<br/>Retrieval · Authority · Citations<br/>Grounding · Safety · Completeness"]

    O["Results<br/>JSON · CSV · Markdown"]
    C["Cross-Model<br/>Comparison"]
